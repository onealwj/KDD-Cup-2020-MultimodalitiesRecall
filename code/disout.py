    ...
    def forward(self, x):
        ...
        if not self.training: 
            ...
            if x.dim()==3:           
                ...
                seed_drop_rate = self.dist_prob* (width) / self.block_size**2 / ( width -self.block_size + 1)
                
                
                valid_block_center=torch.zeros(width,device=x.device).float()
                valid_block_center[int(self.block_size // 2):(width - (self.block_size - 1) // 2)]=1.0
                ...
                if self.block_size == width:            
                    block_pattern = torch.min(block_pattern.view(x.size(0),x.size(1),x.size(2)),dim=2)[0].unsqueeze(-1).unsqueeze(-1)
                else:
                    block_pattern = -F.max_pool2d(input=-block_pattern, kernel_size=(1, self.block_size), stride=(1, 1), padding=(0, self.block_size // 2))
                ...
                if not (self.weight_behind is None) and not(len(self.weight_behind)==0):
                    wtsize=self.weight_behind.size(2)
                    ...
                    weight_mean=weight_max.mean(dim=(1,2),keepdim=True)
                    if wtsize==1:
