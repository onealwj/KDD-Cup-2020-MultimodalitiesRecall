...
from evaluation import i2t, t2i, AverageMeter, LogCollector, encode_data, nDCG5_t2i_rerank, nDCG5_t2i, nDCG5_t2i_atten, nDCG5_t2i_atten_rerank
import logging
import sys
import argparse
...
def main():
    ...
    #vocab = deserialize_vocab(os.path.join(opt.vocab_path, 'kdd2020_caps_vocab_train.json'))
    ...
    train_loader, val_loader = data.get_loaders(
        opt.data_name, vocab, stopwords, opt.batch_size, opt.workers, opt, False)
    ...
    if opt.resume:
        if os.path.isfile(opt.resume):
            ...
            best_rerank_rsum = checkpoint['best_rerank_rsum']
            model.load_state_dict(checkpoint['model'])
            # Eiters is used to show logs as the continuation of another
            # training
            model.Eiters = checkpoint['Eiters']
            print("=> loaded checkpoint '{}' (epoch {}, best_rsum {}, best_rerank_rsum {})"
                  .format(opt.resume, start_epoch, best_rsum, best_rerank_rsum))
...
def validate(opt, val_loader, model):
     ...
     logging.info("Text to image: %.5f %.5f" % (org_ndcg5, rerank_ndcg5))
     '''
     with torch.no_grad():
         q_id_dict = encode_data(
             model, val_loader, opt.log_step, logging.info)
