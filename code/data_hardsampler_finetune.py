...
class TrainDataset(data.Dataset):
    ...
    def initial(self):
        ...
        tem_samples_new = []
        for i in range(len(self.samples)):
            tem_samples_new.append(self.samples.iloc[i])
        random.shuffle(tem_samples_new)

        s = []
        for i in range(len(self.samples)):
            #data = self.samples.iloc[i]
            data = tem_samples_new[i]
            query_id = data['query_id']
            if str(query_id)  in s:
                continue
            else:
                s.append(str(query_id))
                self.samples_new.append(tem_samples_new[i])
        ...
        self.t_dict = {}
        for i, e in enumerate(self.samples):
            _caption = e['query']
            _tokens = nltk.tokenize.word_tokenize(
                str(_caption).lower().encode('utf-8').decode('utf-8'))
            t = _tokens[-1]
            if self.t_dict.__contains__(t):
                self.t_dict[t].append(e)
            else:
                self.t_dict[t] = []
                self.t_dict[t].append(e)

        tem_dict = {}
        for k,v in self.t_dict.items():
            random.shuffle(v)
            tem_dict[k] = v
        self.t_dict = tem_dict
        ...
        for k,v in self.t_dict.items():
            ...
            for j in range(len(v)):
                a.append(v[j])
                if (j+1) % self.max_same==0:
                    if kk <=5:
                        tem_dict[k+'_'+str(kk)] = a
                    kk += 1
                    flag = True
                    a = []
            if not flag:
              if len(v) > 1:
                tem_dict[k] = v
        self.t_dict = tem_dict
        #print(self.t_dict.keys())

        self.key_list = list(self.t_dict)
        self.key = random.choice(self.key_list)

        self.index = list(range(len(self.other)))
        random.shuffle(self.index)
        '''
        print(self.t_dict.keys(), len(self.t_dict))
        len_list = []
        for k,v in self.t_dict.items():
        ...
        self.dict_sample_num = 0
        for k,v in self.t_dict.items():
            self.current_sample_num += len(v)
            self.dict_sample_num += len(v)
        #self.current_sample_num += len(self.other)
        #if self.shuffle:
        #    random.shuffle(self.index)

    def __getitem__(self, index):

        t_list = self.t_dict[self.key]
        data = t_list.pop()
        self.t_dict[self.key] = t_list
        if len(t_list) == 0:
