...
class TrainDataset(data.Dataset):
    ...
    def initial(self):
        ...
        self.current_sample_num = self.nraws
        self.num_load = 0
        self.index = list(range(self.num_load, self.current_sample_num+self.num_load))
        self.num_load += self.nraws
        if self.shuffle:
    ...
    def __getitem__(self, index):
        ...
        '''
         #---py
        #---py #stop and exchange token places
        exchange_flag = 0
        rr=random.random()
        if rr>=0.6: #do exchange
              for token in tokens:
                  rrr=random.random()
                  if (rrr>=0.6) and (exchange_flag==0) and (len(caption)>=3):
                      #exchange this token with its former one
                      if token not in self.stopwords:
                          exchange_flag=1
                          #c_old = caption
                          caption_=caption[:-1]
                          caption_.append(vocab(token))
                          caption_.append(caption[-1])
                          caption=caption_
                          #print(c_old, caption)
                  else:
                      if token not in self.stopwords:
...
class ValidDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab, stopwords, mode=1):
        self.vocab = vocab
        self.loc = data_path + '/'
        self.data_split = data_split
    ...
    def __getitem__(self, index):
        ...
        for token in tokens:
            if token not in self.stopwords:
                ...
                '''
                if token == 'forest' and tmp== '':
                    tmp = 'forest'
                if token == 'style' and tmp == 'forest':
                    token = 'department'
                    caption_mid[-1] = vocab('sen')
                    tmp = ''
                '''

                caption_mid.append(vocab(token))
        tmp = ''

        if self.mode == 1:
            caption_mid = caption_mid + caption_mid 
        if self.mode == 2:
            caption_mid = caption_mid + caption_mid[::-1] 
        #caption_mid = caption_mid 
...
class TestDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab, stopwords, mode=0):
        self.vocab = vocab
        self.loc = data_path + '/'
        self.data_split = data_split
        self.reader = pd.read_csv(self.loc+'%s/testB.tsv' % self.data_split, sep='\t', quoting=csv.QUOTE_NONE)
        self.length = self.reader.shape[0]
        self.stopwords = stopwords
        self.mode = mode
    ...
    def __getitem__(self, index):
        ...
        for token in tokens:
            if token not in self.stopwords:
                #caption.append(vocab(token)) 
                if token == 'sen' and tmp== '':
                    tmp = 'sen'
                if token == 'department' and tmp == 'sen':
                    token = 'style'
                    caption_mid[-1] = vocab('forest')
                    tmp = ''
                    #print(tokens)
                caption_mid.append(vocab(token)) 
        tmp = ''
...
def get_loaders(data_name, vocab, stopwords, batch_size, workers, opt, only_test=False):
    ...
    if not only_test:
        ...
        train_loader = None
        val_loader = get_test_loader(dpath, 'testB', vocab, stopwords, opt,
                                    batch_size, False, workers)
